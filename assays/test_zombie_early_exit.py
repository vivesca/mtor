...
class TestEarlyExitCleanCommit:
    ...
    def test_translocase_early_exits_on_clean_commit(self, tmp_path):
        ...
        # post_diff must be the _git_snapshot result, not a hardcoded empty dict
...
class TestEarlyExitHeadMovedFallback:
    """When _git_snapshot returns 0 commits but HEAD moved, fallback sets commit_count."""

    def test_fallback_when_snapshot_misses_commit(self, tmp_path):
        """_git_snapshot returns 0 commits but rev-list sees 2 — fallback kicks in."""
        from mtor.worker.translocase import translate
        ...
        def mock_run(cmd, **kwargs):
            ...
            if cmd[0] == "git":
                ...
                if subcmd == "rev-parse":
                    ...
                    if rev_parse_count == 1:
                        return subprocess.CompletedProcess(cmd, returncode=0, stdout="aaa111\n", stderr="")
                    return subprocess.CompletedProcess(cmd, returncode=0, stdout="ccc333\n", stderr="")
                if subcmd == "status":
                    return subprocess.CompletedProcess(cmd, returncode=0, stdout="", stderr="")
                if subcmd == "diff":
                    return subprocess.CompletedProcess(cmd, returncode=0, stdout="", stderr="")
                ...
                if subcmd == "commit":
                    return subprocess.CompletedProcess(cmd, returncode=0, stdout="", stderr="")
                if subcmd == "rev-list":
                    # Simulate 2 commits between pre_sha and HEAD
        ...
        task = "---\ntests:\n  - assays/test_zombie_early_exit.py\n---\n[t-fb001] Fix fallback"

        mock_info = MagicMock()
        mock_info.workflow_id = "test-wf-fb001"

        # Snapshot returns 0 commits (simulating _git_snapshot missing them)
        snapshot_result = {"stat": "", "numstat": "", "commits": [], "commit_count": 0, "patch": ""}
        ...
        with patch("mtor.worker.translocase._subprocess.run", side_effect=mock_run), \
             ...
             patch("mtor.worker.translocase._detect_prior_commits", return_value=[]), \
             patch("mtor.worker.translocase._git_pull_ff_only"), \
             patch("mtor.worker.translocase._git_snapshot", return_value=snapshot_result):
        ...
        assert result["success"] is True
        assert result.get("verdict") == "early_exit_clean"
        # The fallback must have set commit_count from rev-list
        assert result["post_diff"]["commit_count"] == 2
        assert result["post_diff"].get("head_moved_fallback") is True
